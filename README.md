...
Den (Apple TV) ──GET /manifest.json──►  atlas   { resources: ["dataset"] }
               ...
               ──GET /index/similar/…─►         { titles }     ← ranked here, off the mmap'd store
...
| `GET /index/similar/<movie\|series>/<tmdbId>.json` | with `INDEX_QUERIES` on: `{titles:[{type,id}],total}` for More Like This (`skip`/`limit` page it) — films and series in one row, each named with its type |
...
| `POST /index/suggest.json` | with `INDEX_QUERIES` on: `{seeds (≤8),exclude?,limit?}` → `{perSeed:[{seed,titles}],pooled}`, More Like This per seed and pooled in seed order, every title `{type,id}` |
