...
| `GET /index/query.json?q=&type=&skip=&limit=` | with `INDEX_QUERIES` on: search in one request — `{parse,hits:[{type,id,score,title,posterPath,year,genreIds,originalLanguage?,f}],total}`. The query is read for a country, decade, type, genre, label or plot facet, and every candidate (fuzzy title, facet, label, plot facet, plot vectors on the leftover) is scored `Φ·[2.0·title + w·semantic + 0.25·label + 0.10·plotFacet + 0.15·popularity]`, so an exact title always outranks a theme match; an exact title leads its More Like This. 40 a page, at most 100 |
