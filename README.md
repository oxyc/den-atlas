...
| `GET /index/facets/<movie\|series>.json?sel=<kind>:<id>,…` | with `INDEX_QUERIES` on: facet counts for stackable filters — `{<kind>:{<id>:count}}`, for every value of every kind, the titles of the type carrying the whole selection AND that value. Den Web hides an option whose count is 0. Kinds: `genre` (TMDB genre id; series also answer under TMDB's composites 10759/10765/10768), `language` (ISO 639-1, lowercase), `country` (ISO 3166-1, uppercase), `decade` (its first year, `1990`), `mood` and `subgenre` (label names at ≥ 0.55, as `/index/row` takes them) and each plot-facet axis by name (`tone:bleak`, `ending:bittersweet`; `structure` resolves as `/index/row` resolves it, and the merged rows `ending:unresolved`, `ending:unhappy` and `chronology:out-of-order` count the union of their members). Zero counts are left out: a value missing from a kind that is present counts 0. A kind whose source did not load (the facts for genre/language/country, the facet rows for the axes) is absent, and a selection naming one answers `{}` — "can't say", never zeros. Every language and country a title lists counts, so a count errs high rather than hiding an option. The URL is the cache key: canonical is `sel` alone (no query at all for an empty selection), ids normalised as above, sorted by kind then id as strings, each once, `sel` percent-encoded whole or per id; anything else is a `308` to the canonical spelling (relative `Location`). At most 16 values and a 2,048-byte query, else `400` `{error:"bad_request",detail}`, as is an unknown kind, a missing `:` or a non-integer genre/decade. Bitsets built at index load (~50 ms, ~2.6 MB); an answer is well under 1 ms. Cached like the other `/index/…` GETs (content ETag, `304`) |
| `GET /index/browse/<movie\|series>.json?sel=<kind>:<id>,…&skip=&limit=` | with `INDEX_QUERIES` on: the titles carrying EVERY selected value (two genres are both genres), most voted first, as `{titles,total}` — each title the same card `/index/row` returns (`type,id,title,posterPath,year,genreIds,primaryGenre,imdbId?,originalLanguage?`), 24 a page, at most 100. No `sel` lists every title of the type, most voted first. `sel` is `/index/facets/…`'s: same kinds, same canonical form, same caps, same `400`s; `skip` and `limit` follow it in that order, each only when not its default (0, 24), and anything else — a `limit` over 100 or under 1 included — is a `308` to the canonical spelling. A kind whose source did not load matches nothing. The order is worked out once per type and selection and each page is a slice of it (a 3-value selection orders in well under 1 ms, every title of a type in ~25 ms); cached like the other `/index/…` GETs |
