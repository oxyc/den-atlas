...
| `GET /playground` | with `PLAYGROUND` and `INDEX_QUERIES` on: the tuning page. `/playground/params.json` lists every More Like This knob (`den_index::SimilarParams`) with its range and production value; `/playground/similar/<movie\|series>/<tmdbId>.json?<knob>=…&limit=` ranks with those overrides and returns each title's score, production rank and per-signal points. The ranges bound one request's cost (`pool_k ≤ 1000`, `max_row ≤ 400`, `limit ≤ 200`). An unknown knob or out-of-range value is a `400` naming it; `no-store`; off ⇒ `404` |
