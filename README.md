...
| `GET /index/awards.json` | with `INDEX_QUERIES` on: every award ceremony or awarding body a title here was recognised at — `{ceremonies:[{id,name,movies,series,won:{movies,series}}]}`, most titles first; each one's row is `sel=award:<id>`, its winners' `sel=won:<id>`. Empty for a store without the award sections |
...
| `GET /index/title/<movie\|series>/<tmdbId>.json` | with `INDEX_QUERIES` on: one title as the corpus describes it — the card `/index/row` draws, `indexed`, `labels` and `plotFacets` at the display floor, and the facts' countries, languages, runtime, `basedOn`, `makers` and `cast` (`{id: Q-id, name, tmdbId?}`, cast at most 60 with `castTotal`), and `awards` (`{id: ceremony Q-id, name, won}`) when it has any. A title with no card is `{type, id, indexed: false}` |
...
| `POST /index/suggest.json` | with `INDEX_QUERIES` on: `{seeds (≤8),exclude?,limit?}` → `{perSeed:[{seed,ids,mixed}],pooled,pooledMixed}`, seed-focused structural affinity per seed and pooled in seed order, with More Like This appended for coverage and as an old-store fallback; `mixed` and `pooledMixed` include films and series together, `{type,id}` |
...
- **People** (`people.json`, `people/counts.json`; `src/filter/people.rs`) answer who is credited on the titles
  ...
  without one it is left out rather than picked by TMDB id. `people/counts.json` lists an item trait's top 30
  values; `people/values/<trait>.json` counts every value the same way and finds one by name or alias
  (`citizenship` `q=iceland`), for a value past the top.
