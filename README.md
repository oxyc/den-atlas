...
| `GET /index/title/<movie\|series>/<tmdbId>.json` | with `INDEX_QUERIES` on: one title as the corpus describes it — the card `/index/row` draws, `indexed`, `labels` and `plotFacets` at the display floor, and the facts' countries, languages, runtime, `basedOn`, `makers` and `cast` (`{id: Q-id, name, tmdbId?}`, cast at most 20 with `castTotal`). A title with no card is `{type, id, indexed: false}` |
