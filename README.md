...
| `GET /index/filter/<movie\|series\|all>/people.json?sel=…&traits=…&order=&skip=&limit=` | with `INDEX_QUERIES` on: the people credited on the titles carrying `sel`, holding every person trait in `traits` (see **People** below), in `order`: `prominence` (the default), `credits`, `name`, `born_asc` or `born_desc` — `{people: [{id, name, tmdbId?, credits, roles, gender?, born?, died?, citizenship?, occupation?}], total, order, orderUnavailable?, labels, coverage, ignored, ignoredTraits?, unknownTraits?, …}`; paged as `titles.json`. `labels` names every trait id on the page |
