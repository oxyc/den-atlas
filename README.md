...
- **`all`** asks every question of films and series together: counts and values over both, titles in one
  popularity order over both (the same measure each type's order uses, so each type keeps its own order within
  it). Every kind answers there; one only a type has (`network`, a series-only genre such as 10762 kids, a
